...
async function main() {
  ...
  const director = await prisma.user.upsert({ where: { email: 'director@elayone.org' }, update: {}, create: { name: 'Alain Mbonyumucyo', email: 'director@elayone.org', passwordHash, role: Role.ADMIN } });
